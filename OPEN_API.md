# 小火龙工具箱 · 开放平台接口文档（第三方接入）

> 面向第三方开发者：通过 **API Key** 调用扫码确认等开放接口，实现与「小火龙扫码登录器」相同能力的自动登录。
> 相比客户端接口，开放接口**无需加密传输、无需登录**，以 `xhlkey` 请求头鉴权，普通 JSON 返回。

---

## 1. 申请 API Key

1. 登录管理后台（管理员 / 超级管理员）。
2. 顶部选择项目 → 左侧「卡密管理 → API 密钥」。
3. 点「创建 Key」，填写名称与备注，保存后**立即复制**生成的 key（仅创建时可见一次）。

### 1.1 Key 格式与项目绑定

- 格式：`sk-` 前缀 + 32 位十六进制字符，示例 `sk-56f50dc7d387f7a1352a06a42758128f`。
- **按项目绑定**：每个 key 属于一个项目（如 `100001`），调用开放接口时**只作用于该项目**。
- 可在后台禁用 / 删除 key，禁用后调用立即失败（返回 1002）。

### 1.2 鉴权方式

所有开放接口在请求头携带：

```
xhlkey: sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```

- 缺少或无效 key → 返回 `1002`。
- 被禁用的 key → 同样返回 `1002`。

---

## 2. 基础约定

### 2.1 基础地址

| 环境 | Base URL |
|---|---|
| 生产 | `http://103.36.223.143:8888` |

### 2.2 响应结构

统一 JSON（HTTP 200，业务结果用 `code` 区分）：

```json
{ "code": 0, "message": "ok", "data": { } }
```

### 2.3 业务错误码

| code | 含义 |
|---|---|
| 0 | 成功 |
| 1001 | 参数错误 |
| 1002 | 缺少 / 无效 / 已禁用的 xhlkey |
| 1004 | 项目不存在或已停用 |
| 1016 | 网络环境加载失败（代理拉取失败） |

---

## 3. 扫码确认

### 3.1 接口说明

```
POST /api/open/qrlogin
xhlkey: sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
Content-Type: application/json
```

> 功能：把手机百度 App 上**已扫码待确认**的二维码，用目标百度账号的 cookie 代为确认登录。
> 一次调用确认一个二维码，同步返回结果。

### 3.2 请求参数

```json
{
  "qrUrl": "https://wappass.baidu.com/wp/?qrlogin&sign=xxxx&lp=pc",
  "ck": "BDUSS=xxx; STOKEN=xxx"
}
```

| 参数 | 必填 | 说明 |
|---|---|---|
| `qrUrl` | 是 | 二维码内容（登录链接，须含 `sign`；可选 `lp`） |
| `ck` | 是 | 目标百度账号的 cookie 串，**至少包含 `BDUSS`**（缺 BDUSS 将确认失败） |

### 3.3 成功响应

```json
{
  "code": 0,
  "message": "ok",
  "data": {
    "ok": true,
    "errno": "0",
    "code": "0",
    "message": ""
  }
}
```

`data` 字段（百度 sapi 结果）：

| 字段 | 说明 |
|---|---|
| `ok` | `true` 确认成功；`false` 确认失败（二维码被消费 / 凭证失效 / 风控等） |
| `errno` | 百度返回的 errno（字符串） |
| `code` | 百度返回的 code |
| `message` | 百度返回的 message / error_msg（可为空） |

> 确认**成功**后，该账号的 cookie 会写入系统的 cookie 库（后台「cookie库」可查看 / 导出），
> 归属为第三方来源，账号密码以随机值占位（真实身份由 cookie 决定）。

### 3.4 失败响应

- 参数 / 鉴权 / 项目校验失败（HTTP 200，`code` 非 0）：

```json
{ "code": 1002, "message": "缺少 xhlkey 请求头" }
```

- 百度确认失败：HTTP 200，`code=0` 但 `data.ok=false`：

```json
{
  "code": 0,
  "message": "ok",
  "data": { "ok": false, "errno": "-1", "code": "400202", "message": "二维码已过期" }
}
```

---

## 4. 调用示例

### cURL

```bash
curl -X POST http://103.36.223.143:8888/api/open/qrlogin \
  -H "xhlkey: sk-56f50dc7d387f7a1352a06a42758128f" \
  -H "Content-Type: application/json" \
  -d '{"qrUrl":"https://wappass.baidu.com/wp/?qrlogin&sign=xxxx&lp=pc","ck":"BDUSS=xxx; STOKEN=xxx"}'
```

### Python

```python
import requests

url = "http://103.36.223.143:8888/api/open/qrlogin"
headers = {"xhlkey": "sk-56f50dc7d387f7a1352a06a42758128f"}
data = {
    "qrUrl": "https://wappass.baidu.com/wp/?qrlogin&sign=xxxx&lp=pc",
    "ck": "BDUSS=xxx; STOKEN=xxx",
}
resp = requests.post(url, json=data, headers=headers, timeout=30).json()
if resp["code"] == 0 and resp["data"]["ok"]:
    print("登录确认成功")
else:
    print("失败:", resp.get("message") or resp["data"].get("message"))
```

---

## 5. 注意事项

1. **BDUSS 必填**：`ck` 中缺少 `BDUSS` 时确认会失败（`data.ok=false`）。
2. **二维码一次性**：一个二维码确认成功后即被消费，再次确认同一 `qrUrl` 会失败；应在每次检测到新二维码时传最新的 `qrUrl`。
3. **代理池**：后端全局配置了代理池（后台「系统设置 → 代理池配置」）时，接口会通过代理访问百度；未配置则直连。
4. **速率**：接口未做独立限流，但请勿高频空跑；异常调用会消耗百度侧风控额度。
5. **cookie 归属**：确认成功写入 cookie 库的记录归属第三方（`user_id=0`），与客户端用户写入的记录区分。
6. **权限**：key 只对绑定项目有效；同一项目下可创建多个 key 分发给不同第三方。
